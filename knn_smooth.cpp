...
// Similar to R's `dist(method = "euclidean")` function
// Ref: http://blog.felixriedel.com/2013/05/pairwise-distances-in-r/
// [[Rcpp::export]]
...
// [[Rcpp::export]]
NumericMatrix calculate_distances(NumericMatrix X, int verbose=0){
  // Input:
  // - X: a matrix in a shape of #genes x #samples
...
NumericMatrix aggregate_k_nearest(
  ...
  //  If not specified, D is the distance matrix of the input Xr.
  // - k: an integer specifying the k nearest samples (self-inclusive) to choose.
  // Return:
...
NumericMatrix knn_smoothing(NumericMatrix Xr,
  ...
  for (int p = 1; p < num_powers + 1; p++){
    ...
    NumericMatrix D = calculate_distances(S, verbose);
